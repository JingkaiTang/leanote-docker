...
RUN apk --update add curl && \
    curl -L http://sourceforge.net/projects/leanote-bin/files/2.5/leanote-linux-amd64-v2.5.bin.tar.gz/download >> \
...
RUN hash=$(< /dev/urandom tr -dc A-Za-z0-9 | head -c${1:-64};echo;); \
    ...
    sed -i "s/adminUsername=.*$/adminUsername=\${ADMIN_USERNAME} /" /usr/local/leanote/conf/app.conf;
EXPOSE 9000
